...
public class BaseKlaxon {
    ...
    public static void stop(Context context) {
        if (sStarted) {
            ...
            sAudioManager.setStreamVolume(sStream,
                    sSavedVolume, 0);
    ...
    public static void start(final Context context, int stream, boolean incVol, long incVolSpeed, boolean shuffle, int vol, Uri alarmNoise, boolean vibrate) {
        sContext = context;
        ...
        LogUtils.v("BaseKlaxon.start() ");

        sVolumeIncreaseSpeed = incVolSpeed;
        ...
        // save current value
        sSavedVolume = sAudioManager.getStreamVolume(sStream);
        sIncreasingVolume = incVol;
        sRandomPlayback = shuffle;
        sFirstFile = true;
        sStarted = true;


        sMaxVolume = calcNormalizedVolume(context, vol);
                ...
                try {
                    ...
                    if (Utils.isStorageUri(alarmNoise.toString())) {
                        if (Utils.isStreamM3UFile(alarmNoise.toString())) {
                            // don't check for network right away cause device might need
                            // a little bit after waking up - so rely on timeout of media player
        ...
        } else if (Uri.EMPTY.equals(alarmNoise)) {
            // silent
            alarmNoise = null;
        }
    ...
    private static void startAlarm(final Context context, MediaPlayer player) throws IOException {
        ...
        LogUtils.v("Using audio stream " + (sStream == AudioManager.STREAM_MUSIC ? "Music" : "Alarm"));
        player.setAudioStreamType(sStream);
        sAudioManager.requestAudioFocus(null, sStream,
